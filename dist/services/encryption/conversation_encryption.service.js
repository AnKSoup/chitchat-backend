...
export function createKeyAndIV() {
    try {
        ...
        return iro(true, "Key and IV created successfully.", 201, "Key and IV in content.", { key: key, iv: iv });
    }
    catch (error) {
...
export function encrypt(message, key, iv) {
    try {
        ...
        return iro(true, "Message encrypted successfully.", 200, "Message and tag in content.", { message: result, tag: tag });
    }
    catch (error) {
...
export function decrypt(message, key, iv, tag) {
    try {
        ...
        return iro(true, "Message decrypted successfully.", 200, "Message in content.", { message: result });
    }
    catch (error) {
        //TODO ADD CHECKS TO KNOW IF ISSUE IS FROM THE KEY OR THE IV
        const message = getProperty("message", error);
