...
async function dbUnlock(db) {
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            const stmt = db.prepare(`PRAGMA key = ${dbPass}`);
            stmt.run((err) => {
                if (err) {
                    reject(iro(false, "Couldn't connect to the database.", 500, err.message));
                }
                else {
                    resolve(iro(true, "Connected to database.", 100, "Password is correct."));
                }
            });
            stmt.finalize();
        });
...
async function dbRun(query, object, message_success) {
    //Connect to db:
    const db = dbOpen();
    try {
        await dbUnlock(db);
...
export async function dbSelectJoin(columns1, table1, columns2, table2, join1, join2, conditions, invert, limit, offset) {
    ...
    let query = `SELECT ${columns1
        ...
        .join(", ")} FROM ${table1} JOIN ${table2} ON ${table1}.${join1} = ${table2}.${join2}`;
    if (conditions) {
        query += ` WHERE ${conditions.join(" AND ")}`;
    }
    if (invert) {
...
export async function dbInsert(table, object) {
    ...
    return await dbRun(query, object, `Operation successful: added object to ${table}.`);
}
// UPDATE: await dbUpdate()
export async function dbUpdate(table, object, conditions) {
    ...
    for (let i = 0; i < Object.keys(object).length; i++) {
        if (i < Object.keys(object).length - 1) {
            ...
            query += `${Object.keys(object)[i]} = ? WHERE ${conditions.join(" AND ")};`;
        }
    }
    return await dbRun(query, object, `Operation successful: updated object to ${table}.`);
}
// DELETE:
export async function dbDelete(table, conditions) {
    const query = ` DELETE FROM ${table} WHERE ${conditions.join(" AND ")};`;
    return await dbRun(query, undefined, `Operation successful: deleted object from ${table}.`);
}
