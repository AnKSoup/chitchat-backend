...
import { createItem, deleteItem, getItemsJoin, updateItem, } from "./safe_queries.service.js";
//Gets all the message of a conversation within a given range
export async function getMessagesFromConv(conversation_id, count, offset) {
    return await getItemsJoin([
        ...
        "message_modified_at",
        "in_response_to",
    ], "Message", ["user_id", "user_name"], "User", "user_id", "user_id", [`conversation_id = ${conversation_id}`], "message_id", count, offset);
}
...
export async function createMessage(message, conversation_id) {
    ...
    let in_response_to = getProperty("in_response_to", message);
    // Checks if message even exists in conversation
    const test = await isMessageInConv(in_response_to, conversation_id);
    if (in_response_to == 0) {
        in_response_to = null;
    }
    else if (!getSuccess(test)) {
        return iro(false, "Cannot reply to message.", 400, "Message not in conversation.");
    }
...
export async function editMessage(message_id, message_content) {
    const date = new Date();
    return await updateItem("Message", {
        message_content: message_content,
...
export async function isMessageOfId(message_id, user_id) {
...
export async function isMessageInConv(message_id, conversation_id) {
    return await doesItemExist(["message_id"], "Message", [
        `message_id = ${message_id}`,
        `conversation_id = ${conversation_id}`,
    ]);
